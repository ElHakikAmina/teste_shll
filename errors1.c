 ...
 *        -1 on error
 */
void print_error(info_t_struct *info, char *estr)
{
	_eputsfnc(info->filename);
	_eputsfnc(": ");
	print_d(info->line_count, STDERR_FILENO);
	_eputsfnc(": ");
	_eputsfnc(info->argv[0]);
	_eputsfnc(": ");
	_eputsfnc(estr);
}
...
char *convert_number(long int num, int base, int flags)
{
	...
	array = flags & CNVRT_LOWERCASE ? "0123456789abcdef" : "0123456789ABCDEF";
	ptr = &buffer[49];
	*ptr = '\0';
